...
SDL_Surface *screen, *gfx, *gfxo;
extern pDesc player[2];
SDL_Joystick *joy[2]={ NULL, NULL };
SDL_Event event;
Uint32 tick, ntick, ttick1,ttick2, ttick3, ttick4, ttick5, ttick6;
	...
	for(done=false,tick=SDL_GetTicks();!done && afterdeath<400;) {
		...
		if(ntick-tick>=1000/FPS) {
			...
			if(!(player[0].shield))
				afterdeath++;
			//ttick4=SDL_GetTicks();
			//fprintf(stderr,"Character engine %i\n",ttick4-ttick3);	
			engine_obj();
			//ttick5=SDL_GetTicks();
			//fprintf(stderr,"object engine %i\n",ttick5-ttick4);
			engine_vefx();
			// panel 
...
main (int argc, char *argv[])
{
	...
	if(conf.fullscreen)
		screen=SDL_SetVideoMode(SCREENW, SCREENH, 16,0);//bpp,SDL_HWSURFACE|SDL_HWPALETTE|SDL_DOUBLEBUF|SDL_FULLSCREEN);
	else
		screen=SDL_SetVideoMode(SCREENW, SCREENH, 16,0); //bpp,SDL_HWSURFACE|SDL_HWPALETTE|SDL_DOUBLEBUF);
