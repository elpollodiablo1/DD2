...
engine_fire()
{
	...
	for(ftp=ft=fused;ft;) {
		...
		SDL_BlitSurface(gfx, &b, screen, &a);
		ttick2=SDL_GetTicks();
		fprintf(stderr,"blit 4: %i\n",ttick2-ttick1);
		//SDL_UpdateRect(screen,0,0,0,0);
		//SDL_UpdateRect(screen,a.x,a.y,b.w,b.h);//added by me
		ftp=ft;
		ft=ft->n;
	}
}
...
if(p->cftime<p->ftime)
	...
	if(p->incx) {
		if(p->incx>0) {
			if(p->x+4<SCREENW-40)//p->x+4<SCREENW-40
				p->x+=4;//4
			else {
				p->shield--;
				if(p->shield)
	...
	if(p->incy) {
		if(p->incy>0) {
			if(p->y+4<SCREENH-20)//p->y+4<SCREENH-20
				p->y+=4;//4
		} else {
			if(p->y-4>40)//p->y-4>40
				p->y-=4;//4
		}
	}
...
SDL_BlitSurface(gfx, &b, screen, &a);
ttick2=SDL_GetTicks();
fprintf(stderr,"blit 5: %i\n",ttick2-ttick1);	
//SDL_UpdateRect(screen,a.x,a.y,b.w,b.h);
/* companion */
if(p->companion) {
	a.x=p->cx;
	a.y=p->cy;
