...
SDL_Surface *screen, *gfx, *gfxo, *bg;
extern pDesc player[2];
SDL_Joystick *joy[2]={ NULL, NULL };
SDL_Event event;
Uint32 tick, ntick, ttick1,ttick2, ttick3, ttick4, ttick5, ttick6, ttick7, ttick8;
...
gameLoop()
{
	...
	for(done=false,tick=SDL_GetTicks();!done && afterdeath<400;) {
				...
				else
				{
					if(event.key.keysym.sym==SDLK_p && pause_tick<SDL_GetTicks())
					{
						...
						pause_tick=SDL_GetTicks()+200;
							...
							while(pause){
								SDL_WaitEvent(&event);
								if((event.key.keysym.sym==SDLK_p || SDL_GetMouseState(NULL, NULL)&SDL_BUTTON(1)) && pause_tick<SDL_GetTicks()) {
								writeCString(gfx, screen, 98, 20, "game paused", 0);
								SDL_Flip(screen);
								pause=pause ? false : true;
								pause_tick=SDL_GetTicks()+200;
		...
		if(ntick-tick>=1000/FPS) {
			...
			//fprintf(stderr,"tick %i\n",tick);
			//fprintf(stderr,"ntick %i\n",ntick);
			fprintf(stderr,"everything in between %i\n",ntick-ttick8);
			fprintf(stderr,"one loop %i\n",ntick-tick);
			tick=ntick;
				// scrolling of background 
			{
				...
				if(!scroll) {//&& is==1
					...
					SDL_BlitSurface(gfx, &b, screen, &a);
					//SDL_Flip(bg);
					fprintf(stderr,"actual scroll 1\n");
				}
				ttick1=SDL_GetTicks();
				fprintf(stderr,"scroll 1: %i\n",tick-SDL_GetTicks());
				...
				if(!scroll2) {//&& is==1
					...
					b.h=(int)scroll2;
					SDL_BlitSurface(gfx, &b, screen, &a);
					fprintf(stderr,"actual scroll 2\n");
					//SDL_Flip(bg);	
				}
				fprintf(stderr,"scroll 2: %i\n",ttick1-SDL_GetTicks());
			/*if (is<2)
				is++;
			fprintf(stderr,"is value %i\n",is);
			*/}
			
			//SDL_FillRect(screen,NULL,SDL_MapRGB(screen->format,0,0,0));
			ttick1=SDL_GetTicks();
			fprintf(stderr,"scroll time %i\n",ttick1-tick);	
			// enemy here 
			engine_enemy();
			// fire here 
			ttick2=SDL_GetTicks();
			fprintf(stderr,"engine_enemy %i\n",ttick2-ttick1);
			engine_fire();
			ttick3=SDL_GetTicks();
			fprintf(stderr,"engine_fire %i\n",ttick3-ttick2);
			// character here 
			if(player[0].shield)
			...
			ttick4=SDL_GetTicks();
			fprintf(stderr,"engine_player %i\n",ttick4-ttick3);	
			engine_obj();
			ttick5=SDL_GetTicks();
			fprintf(stderr,"engine_obj %i\n",ttick5-ttick4);
			engine_vefx();
			
			// panel 
			drawPanel(gfx,screen,player);
			ttick6=SDL_GetTicks();	
			fprintf(stderr,"engine_vefx & drawPannel %i\n",ttick6-ttick5);
			//SDL_UpdateRect(screen,0,0,0,0);
			//SDL_UpdateRect(bg,0,0,0,0);	
			ttick7=SDL_GetTicks();
			//fprintf(stderr,"update BG %i\n",ttick7-ttick6);	
			SDL_Flip(screen);
			ttick8=SDL_GetTicks();
			fprintf(stderr,"sdl_flip(screen) %i\n",ttick8-ttick7);
		}
	}
}
...
main (int argc, char *argv[])
{
	...
	if(vi)
		...
		fprintf(stderr,"bitperpixel: %i\n",bpp);
	if(conf.fullscreen){
		screen=SDL_SetVideoMode(SCREENH, SCREENW, 0,0);//bpp,SDL_HWSURFACE|SDL_HWPALETTE|SDL_DOUBLEBUF|SDL_FULLSCREEN);
		//bg=SDL_SetVideoMode(SCREENW, SCREENH, bpp,0);
	}else {
		screen=SDL_SetVideoMode(SCREENW, SCREENH, 0,0); //bpp,SDL_HWSURFACE|SDL_HWPALETTE|SDL_DOUBLEBUF);
		//bg=SDL_SetVideoMode(SCREENW, SCREENH, bpp,0);	
	}
	if(!screen) {
	...
	gfx=loadBMP(buffer);
	//gfxo=IMG_Load(buffer);
	//gfx=SDL_DisplayFormatAlpha(gfxo);
	if(!gfx) {
		fprintf(stderr,"Unable load gfx: %s\n", SDL_GetError());
		return 1;
	} 
	SDL_FreeSurface(gfxo);
	
	/* set transparent color */
	/*if(SDL_SetColorKey(gfx, SDL_SRCCOLORKEY | SDL_RLEACCEL, SDL_MapRGB(gfx->format, 255, 0, 255))<0) {//gfx, SDL_SRCCOLORKEY, SDL_MapRGB(gfx->format, 255, 0, 255
